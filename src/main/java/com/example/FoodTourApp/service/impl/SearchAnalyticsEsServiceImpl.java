...
public class SearchAnalyticsEsServiceImpl implements SearchAnalyticsEsService {
    ...
    private static final String INDEX = "search_analytics";
    private static final DateTimeFormatter ISO_FMT = DateTimeFormatter.ISO_DATE_TIME;
    private static final int RETENTION_DAYS = 90;
    /** Dedup window: bỏ qua search event trùng (session + query) trong vòng 10 giây.
     *  10 giây đủ để chặn spam từ loadProducts() bị gọi nhiều lần khi token load/screen resume,
     *  nhưng không quá dài để block intentional repeated searches. */
    private static final long DEDUP_WINDOW_MS = 10_000L;

    private final RestTemplate restTemplate = new RestTemplate();
    private final ObjectMapper objectMapper;
    /** In-memory dedup cache: key = sessionId:queryNormalized → timestamp lần log gần nhất. */
