...
public class FileStorageService {
    ...
    public String storeChatFile(MultipartFile file, Integer userId, Long conversationId) throws IOException {
        ...
        Path targetPath = targetFolder.resolve(filename);
        try {
            // Encrypt file bytes trước khi ghi ra disk
            byte[] plainBytes = file.getBytes();
            byte[] encryptedBytes = encryptionService.encryptBytes(plainBytes);
            Files.write(targetPath, encryptedBytes);
            log.info("Chat file saved (encrypted) to {}: {}", targetDir, targetPath);
        } catch (IOException e) {
