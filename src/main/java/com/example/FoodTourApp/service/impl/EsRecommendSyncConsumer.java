...
public class EsRecommendSyncConsumer {
    ...
    @org.springframework.beans.factory.annotation.Value("${elasticsearch.host}")
    private String esHost;

    @org.springframework.beans.factory.annotation.Value("${elasticsearch.port}")
    private int esPort;

    @org.springframework.beans.factory.annotation.Value("${elasticsearch.recommend-index}")
    ...
    private String getEsUrl() { return "http://" + esHost + ":" + esPort; }
        ...
        for (Product product : products) {
            ...
            try {
                Map<String, Object> action = Map.of(
                        "index", Map.of("_index", esIndex, "_id", String.valueOf(product.getId())));
    ...
    private void deleteProducts(List<Integer> productIds) {
        ...
        for (Integer id : productIds) {
            try {
                Map<String, Object> action = Map.of(
                        "delete", Map.of("_index", esIndex, "_id", String.valueOf(id)));
                bulk.append(objectMapper.writeValueAsString(action)).append("\n");
            } catch (Exception e) {
        ...
        try {
            ...
            log.info("Recommend sync: deleted {} products from {}", productIds.size(), esIndex);
        } catch (Exception e) {
