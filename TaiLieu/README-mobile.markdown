# 🍜 Food Tour Platform - Hướng Dẫn Ứng Dụng Di Động

Chào mừng bạn đến với **Food Tour Platform**, một ứng dụng di động Android được xây dựng bằng **Jetpack Compose** và **Kotlin**, chuyên về thương mại điện tử đồ ăn, kết nối người dùng với các cửa hàng địa phương. README này cung cấp thứ tự thực hiện API và công nghệ sử dụng cho từng nhóm API, cùng hướng dẫn cài đặt và cấu hình.
...
## 🎯 Tổng Quan
**Food Tour Platform** là ứng dụng di động Android tối ưu cho trải nghiệm mua sắm đồ ăn trực tuyến, hỗ trợ 4 vai trò: **Admin**, **User**, **Seller**, và **Reseller**. Ứng dụng tích hợp tìm kiếm bản đồ, chat thời gian thực, hệ thống voucher, và thanh toán trực tuyến.

**Điểm Nổi Bật**:
- 🗺️ Tìm kiếm cửa hàng qua bản đồ với GPS.
- 💬 Chat thời gian thực qua WebSocket.
- 🎫 Hệ thống voucher và khuyến mãi linh hoạt.
- 📱 Giao diện mượt mà với Jetpack Compose.
...
Dưới đây là thứ tự gọi API cho các luồng chính trong ứng dụng di động:

### 1. Khởi Tạo Ứng Dụng
1. `GET /api/mobile/app/version` - Kiểm tra phiên bản ứng dụng.
2. `GET /api/mobile/config` - Tải cấu hình (theme, settings).
3. `POST /api/mobile/device/register` - Đăng ký token thiết bị cho thông báo đẩy.
...
8. `GET /api/public/categories` - Danh sách danh mục.
...
| **Nhóm API**                | **Frontend (Mobile)**                 | **Backend**                          | **Database**                   | **Dịch Vụ Bổ Sung**            |
|-----------------------------|---------------------------------------|--------------------------------------|--------------------------------|--------------------------------|
| **Khởi Tạo Ứng Dụng**       | Jetpack Compose, WorkManager, Firebase | Node.js, Express, Firebase Admin   | MySQL, Redis                  | Firebase Cloud Messaging       |
| **Xác Thực**                | Jetpack Compose, DataStore, Retrofit | Node.js, Express, JWT, Nodemailer   | MySQL, Redis                  | SMTP (email)                  |
| **Duyệt & Tìm Kiếm**        | Jetpack Compose, Google Maps SDK, Coil | Node.js, Express, Elasticsearch    | MySQL, Redis                  | Google Maps API               |
| **Quản Lý Hồ Sơ**           | Jetpack Compose, ImagePicker, Retrofit | Node.js, Express, Multer          | MySQL, AWS S3                 | -                             |
| **Quản Lý Địa Chỉ**         | Jetpack Compose, Google Maps SDK     | Node.js, Express, Google Maps API   | MySQL                         | -                             |
| **Mua Sắm & Giỏ Hàng**      | Jetpack Compose, Room, Retrofit      | Node.js, Express, Bull Queue        | MySQL                         | -                             |
| **Đặt Hàng**                | Jetpack Compose, WorkManager, Retrofit | Node.js, Express, Node-cron       | MySQL                         | VNPay, MoMo                   |
| **Voucher**                 | Jetpack Compose, Retrofit            | Node.js, Express                    | MySQL, Redis                  | -                             |
| **Đánh Giá & Bình Luận**    | Jetpack Compose, Retrofit            | Node.js, Express                    | MySQL                         | -                             |
| **Tính Năng Xã Hội**        | Jetpack Compose, Coil, Retrofit      | Node.js, Express                    | MySQL                         | -                             |
| **Chat**                    | Jetpack Compose, OkHttp (WebSocket)  | Node.js, Socket.IO                  | MySQL, Redis                  | -                             |
| **Thông Báo**               | Jetpack Compose, Firebase FCM        | Node.js, Express, Firebase Admin    | MySQL                         | Firebase Cloud Messaging       |
| **Quản Lý Cửa Hàng**        | Jetpack Compose, ImagePicker, Retrofit | Node.js, Express, Multer          | MySQL, Elasticsearch          | AWS S3                        |
| **Hệ Thống Đại Lý**         | Jetpack Compose, QRCode-Kotlin       | Node.js, Express                    | MySQL                         | -                             |
| **Quản Trị**                | Jetpack Compose, Recharts, Retrofit  | Node.js, Express                    | MySQL, Elasticsearch          | -                             |
| **API Chung**               | Jetpack Compose, Retrofit            | Node.js, Express, Multer            | MySQL, Redis                  | AWS S3                        |
| **API Bổ Sung**             | Jetpack Compose, Retrofit            | Node.js, Express                    | MySQL                         | VNPay, MoMo, Twilio           |
